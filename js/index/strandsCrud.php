<?php
    ...
    if(isset($_GET['getData'])){
        $DBCRUDAPI->selectFullleftjoin("strands","departments","department_id","strand_department_id","WHERE strand_department_id=1 ");
        $data = $DBCRUDAPI->sql;
        $res = array();
        ...
        if(isset($_POST['addNew'])){
           ...
           try {
                ...
                $strand_acronym = $_POST["strand_acronym"];
                $strand_department_id = $_POST["strand_department_id"];
                $DBCRUDAPI->insert('strands',['strand_department_id'=>$strand_department_id,'strand_name'=>$strand_name,'strand_acronym'=>$strand_acronym],"strand_id='$strand_id'");
                echo json_encode(array("success"=>true));
            } catch (Exception $th) {
                echo json_encode(array("message"=>$th->getMessage()));
           } 

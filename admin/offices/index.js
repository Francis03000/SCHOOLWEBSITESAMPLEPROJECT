$(document).ready(function () {
  ...
  function getAllData() {
    ...
    $.get({
      ...
      success: function (data) {
        ...
        newData.forEach((offices, i) => {
          dataArray.push(offices);
          let tableRow = $("<tr>", { id: newData.office_id });
          $("<td>", { class: "text-wrap", html: i + 1 }).appendTo(tableRow);
          $("<td>", {
            class: "text-wrap",
            html: offices.office_name,
          }).appendTo(tableRow);
  ...
  $("#create-new").click(function () {
    $("#modalMainLabel").html("Add Office");
  ...
  function update(index) {
    $("#modalMain").modal("show");
    $("#modalMainLabel").html("Update Office");
  ...
  function deletes(id) {
    $.post({
      url: entity + "/" + entity + "Crud.php",
      data: { office_id: id, delete: "delete" },
      success: function (data) {
        data = JSON.parse(data);
        if (data.success) {
          $("#formBodyData").trigger("reset");
  ...
  function view(index) {
    $("#modalMain").modal("show");
    $("#modalMainLabel").html("View offices");
    let models = dataArray[index];
    Object.keys(models).map((key) => {
